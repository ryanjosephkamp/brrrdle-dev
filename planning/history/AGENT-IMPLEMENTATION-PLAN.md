...
**Latest amendment (v3.59, 2026-06-08)**: Phase 23 Stage 18 planning is documented under §28.62 and tracked under `phase_id = 134` as a governance/documentation-only update from `PHASE-23-STAGE-18-MULTIPLAYER-GO-FINAL-PUZZLE-AND-SOLO-PRACTICE-GO-HARD-MODE-FIXES-SPEC-2026-06-08.md`. Stage 18 is a final targeted three-bug pass inside Phase 23: Solo Practice GO Hard Mode checkbox toggling, Multiplayer GO final solved-row hold behavior, and Multiplayer GO final-puzzle no-premature-termination until a correct solve. Future execution must reproduce first, prioritize the final-puzzle termination bug, use small targeted changes, and use real two-client Supabase-backed browser E2E for Practice/Daily Multiplayer GO claims. No source-code edits, tests, UI/component work, Supabase migrations, configuration changes, implementation branch, PR creation, merge, release, Phase 24 work, or Stage 18 execution is authorized by this amendment.

**Latest amendment (v3.60, 2026-06-08)**: Phase 23 Stage 18 execution is explicitly authorized and opened under §28.63 and `phase_id = 135`. The kickoff checkpoint protects the current local `main` worktree with Stage 18 planning/governance dirt, records the baseline process/memory snapshot before dev-server/browser testing, and defines the reproduce-first plan for the three Stage 18 bugs. No source-code fixes have been made at this checkpoint; Stage 18 remains limited to Multiplayer GO final-puzzle no-premature-termination, Multiplayer GO final solved-row hold behavior, and Solo Practice GO Hard Mode checkbox toggling.

**Latest amendment (v3.61, 2026-06-08)**: Phase 23 Stage 18 final verification and handoff are complete under §28.64 and `phase_id = 136`. Stage 18 reproduced and fixed the scoped Multiplayer GO final-puzzle premature-termination bug, verified the final solved-row hold for Practice and Daily Multiplayer GO, and restored Solo Practice GO Hard Mode checkbox toggling with Practice OG behavior preserved. Verification passed focused changed-area tests, lint, 488 full-suite tests, build, API typecheck, diff check, desktop/tablet/390px smoke, real two-client Supabase-backed Practice and Daily Multiplayer GO E2E, remote Supabase cleanup, Solo Practice GO Hard Mode checks, Solo Practice OG non-regression, and final resource checks. No PR, merge, release, production deployment, full dedicated Multiplayer tab work, spectator expansion, scoring/rating changes, broad refactor, Phase 24 work, or out-of-scope work was performed.

**Latest amendment (v3.62, 2026-06-09)**: Phase 23 Stage 19 planning is documented under §28.65 and tracked under `phase_id = 137` as a governance/documentation-only update from `PHASE-23-STAGE-19-SOLO-AND-DAILY-GO-TRANSITION-AND-KEYBOARD-BUGFIXES-SPEC-2026-06-09.md`, with supporting user bug notes in `phase23_stage19_bugs.md`. Stage 19 is a narrow bug-fix-only pass for three GO issues: missing solo Practice/Daily GO solved-row transition screen and sound after correct guesses; Daily GO final-puzzle keyboard coloring rule failures in solo and multiplayer; and asymmetric Multiplayer GO puzzle-4 transition propagation that can leave one player stuck while the other advances. Future execution must reproduce first, make small targeted changes, use real two-client Supabase-backed browser E2E for multiplayer GO claims, and preserve all Stage 12-18 wins plus Daily Multiplayer invariants. No source-code edits, tests, UI/component work, Supabase migrations, configuration changes, implementation branch, PR creation, merge, release, production deployment, Phase 24 work, or Stage 19 execution is authorized by this amendment.

**Latest amendment (v3.63, 2026-06-09)**: Phase 23 Stage 19 execution is explicitly authorized and opened under §28.66 and `phase_id = 138`. The kickoff checkpoint protects the current local `main` worktree with Stage 19 planning/governance dirt, records baseline process/memory state before dev-server/browser testing, and defines the reproduce-first plan for the three Stage 19 bugs. No source-code fixes have been made at this checkpoint; Stage 19 remains limited to solo Practice/Daily GO solved-row transition screen and sound, Daily GO final-puzzle keyboard coloring, and Multiplayer GO asymmetric transition propagation.

**Latest amendment (v3.64, 2026-06-09)**: Phase 23 Stage 19 focused reproduction and fixes are tracked under §28.67 and `phase_id = 139`. Focused regressions reproduced the Multiplayer GO stuck-player propagation bug and Daily Multiplayer GO prior-evidence keyboard-color bug before fixes; browser checks reproduced missing solo Practice/Daily GO solved-row transition holds, while solo Daily GO final-puzzle keyboard coloring was checked and not reproduced. Targeted fixes now recover the non-solving GO player when a shared solved move arrives after that player's canonical board exhausted attempts, derive Daily Multiplayer GO keyboard colors from merged visible GO evidence, and add a solo GO solved-row hold/sound trigger patterned after Multiplayer GO. Focused changed-area tests and solo browser checks pass; full gate, real two-client E2E, remote probes/cleanup, responsive smoke, and final handoff remain pending.

**Latest amendment (v3.65, 2026-06-09)**: Phase 23 Stage 19 final verification and handoff are complete under §28.68 and `phase_id = 140`. Stage 19 fixed the scoped Multiplayer GO stuck-player propagation bug, Daily Multiplayer GO prior-evidence keyboard-color projection, and solo Practice/Daily GO solved-row transition hold/sound trigger. Verification passed focused changed-area tests, wider GO regressions, lint, 493 full-suite tests, build, API typecheck, diff check, desktop/tablet/390px smoke, real browser-backed two-client Supabase Practice/Daily Multiplayer GO E2E, remote Supabase probes/cleanup, solo Practice/Daily GO transition/sound checks, Daily GO keyboard-color checks, and final resource checks. Solo Daily GO final-puzzle keyboard coloring was checked and not reproduced in the solo path, so no solo keyboard helper change was made. No PR, merge, release, production deployment, full dedicated Multiplayer tab work, spectator expansion, scoring/rating changes, broad refactor, Phase 24 work, or out-of-scope work was performed.

**Latest amendment (v3.66, 2026-06-09)**: Phase 23 Stage 20 planning is documented under §28.69 and tracked under `phase_id = 141` as a governance/documentation-only update from `PHASE-23-STAGE-20-MULTIPLAYER-STATUS-TEXT-AND-FORFEIT-LOGIC-BUGFIXES-SPEC-2026-06-09.md`. Stage 20 is an extremely narrow two-bug multiplayer pass: status text synchronization for both players across lobby/join/turn/terminal events, and forfeit win/loss precedence so a forfeiting player loses after any guess has been submitted while pre-guess forfeits may remain non-results/cancellations. Future execution must reproduce first, make small targeted changes, use real two-client Supabase-backed browser E2E plus remote probes/cleanup for multiplayer claims, and preserve all Stage 12-19 wins plus Daily Multiplayer invariants. No source-code edits, tests, UI/component work, Supabase migrations, configuration changes, implementation branch, PR creation, merge, release, production deployment, Phase 24 work, or Stage 20 execution is authorized by this amendment.

**Latest amendment (v3.67, 2026-06-09)**: Phase 23 Stage 20 execution is explicitly authorized and opened under §28.70 and `phase_id = 142`. The kickoff checkpoint protects the clean local `main` state after PR #23, records baseline process/memory state before dev-server/browser testing, and defines the reproduce-first plan for the two Stage 20 bugs. No source-code fixes have been made at this checkpoint; Stage 20 remains limited to multiplayer status text synchronization and forfeit win/loss precedence while preserving timeout behavior, scoring formulas, gameplay surfaces, all Stage 12-19 wins, and Daily Multiplayer invariants.

**Latest amendment (v3.68, 2026-06-09)**: Phase 23 Stage 20 focused reproduction and fixes are tracked under §28.71 and `phase_id = 143`. Focused red tests reproduced the stale multiplayer status text and forfeit-result precedence bugs before source fixes. Targeted fixes now derive the status/message box from shared game state, clear stale local success messages when shared state changes, persist/normalize the forfeiting player, treat pre-guess forfeits as cancellations, and make post-guess forfeits resolve against the forfeiting player before any points fallback while preserving timeout precedence and scoring formulas. Focused changed-area tests and the wider `src/multiplayer` suite pass; real two-client Supabase E2E, remote probes/cleanup, responsive smoke, full automated gate, optional preview, and final handoff remain pending.

**Latest amendment (v3.69, 2026-06-09)**: Phase 23 Stage 20 final verification and handoff are complete under §28.72 and `phase_id = 144`. Real two-client Supabase-backed browser E2E verified synchronized status text and forfeit/timeout/normal terminal behavior across Practice OG, Practice GO, Daily OG, and Daily GO where applicable; remote probes verified durable rows, participants, moves, terminal fields, and cleanup. The full local gate passed (`npm run lint`, `npm run test` with 499 tests, `npm run build`, API typecheck, `git diff --check`), responsive smoke passed at desktop/tablet/390px with no console errors or horizontal overflow, and the Stage 20 dev server was stopped. No PR, merge, release, production deployment, Phase 24 work, or out-of-scope gameplay/scoring/UI work was performed.
...
### 28.62 Stage 18 Planning — Multiplayer GO Final Puzzle Behavior And Solo Practice GO Hard Mode Checkbox Fixes (`phase_id = 134`)

**Status**: Planning/governance-only. Implementation remains gated until the user explicitly authorizes Stage 18 execution.

**Binding source of truth**: `PHASE-23-STAGE-18-MULTIPLAYER-GO-FINAL-PUZZLE-AND-SOLO-PRACTICE-GO-HARD-MODE-FIXES-SPEC-2026-06-08.md`.

Stage 18 is planned as a final targeted bug-fix stage inside Phase 23. It covers exactly three reported defects:

1. **Solo Practice GO Hard Mode checkbox not toggleable**: Solo Practice GO must use the same or equivalent minimal Hard Mode checkbox wiring as Solo Practice OG. The checkbox must toggle, persist the choice, and enforce Hard Mode on the first guess.
2. **Missing final solved-row hold in Multiplayer GO**: after solving the fifth and final puzzle in a Multiplayer GO chain, both Practice Multiplayer GO and Daily Multiplayer GO must show the same brief all-green solved-row hold and transition used after puzzles 1-4 before terminal results/definitions.
3. **Multiplayer GO final puzzle terminating prematurely**: Practice Multiplayer GO and Daily Multiplayer GO must allow alternating turns on puzzle 5 until one player submits the correct answer. The final puzzle must not end after a small number of wrong guesses with a points-only winner.

#### 28.62.1 Strict Scope Boundaries

Stage 18 execution, if later authorized, must stay inside the three defects above. The recommended execution order is:

1. Reproduce and fix Bug 3 first because it breaks the final-puzzle contract and requires real multiplayer evidence.
2. Fix Bug 2 so the final puzzle receives the same solved-row hold/transition treatment as puzzles 1-4.
3. Fix Bug 1 with the smallest Solo Practice GO Hard Mode checkbox wiring change.

Explicitly out of scope:

- Solo Practice OG changes.
- Daily Solo GO changes.
- Any OG-mode changes, including Practice Multiplayer OG and Daily Multiplayer OG.
- Scoring, rating, or ELO rule changes.
- GO chain advancement rules for non-final puzzles.
- Multiplayer GO Hard Mode enforcement changes.
- Customize box behavior, layout, styling, copy, or Stage 17 locking behavior.
- Resume behavior, seed behavior, or the Stage 15 authenticated Practice seed system.
- Broad refactoring of `src/game/go/`, `src/multiplayer/`, session management, or app architecture.
...
If a future root-cause investigation shows that Bug 3 requires broader changes than the narrow surfaces above, execution must stop and report rather than silently expanding scope.

#### 28.62.2 Invariants To Preserve

Stage 18 must preserve all Stage 12 through Stage 17 wins, including:

- Practice Multiplayer Hard Mode enforcement, keyboard responsiveness, sound playback, row-write reduction, stale-save protections, timed Practice behavior, and scoring/result settlement.
- Practice solo one-shot resume behavior, submitted-row animation stability, post-game results visibility, and Multiplayer GO solved-row hold/coordinated advancement.
- Hidden/inert Multiplayer foundations, Calendar/Practice as the active multiplayer entry points, nonparticipant gameplay guard, and the unified `async_multiplayer_games` path.
- GO solved-row hold prior-puzzle visibility, authenticated Practice per-account seeding, and Daily OG/GO global determinism.
- Practice Multiplayer GO prior-solution stack visibility and prior-solution keyboard-state projection.
- Solo Practice GO Customize locking behavior from Stage 17.

Daily Multiplayer must remain strictly asynchronous, fixed at five letters, UTC-day keyed, no-clock, no-Hard-Mode-lobby-control, answer-separated, and claim-safe. `playerSessions` remain canonical per-viewer state, and shared `serializedSession` remains compatibility/answer plumbing only. Solo Practice OG behavior and Daily OG/GO deterministic selection must remain unchanged.

#### 28.62.3 Required Execution Discipline And Verification

If Stage 18 execution is later authorized:

- Reproduce first, especially the Multiplayer GO final-puzzle premature-termination bug with real two-client Supabase-backed browser E2E.
- Make the smallest possible targeted change for each bug.
- Run focused verification immediately after each logical fix before moving to the next item.
- Use real two-client Supabase-backed browser E2E for any Practice or Daily Multiplayer GO claim.
- Pair multiplayer browser evidence with remote Supabase probes and cleanup where relevant.
- Finish with focused changed-area tests, `npm run lint`, `npm run test`, `npm run build`, `npx tsc -p tsconfig.api.json --noEmit`, `git diff --check`, desktop/tablet/390px browser smoke, Solo Practice OG non-regression, Stage 12-17 invariant checks, and resource/process sanity checks.

This planning pass made no source-code, test, UI/component, Supabase migration, configuration, implementation-branch, PR, merge, release, deployment, or Phase 24 changes.

### 28.63 Stage 18 Execution Kickoff — Protected State, Resource Baseline, And Reproduction Plan (`phase_id = 135`)

**Status**: Execution opened. Reproduction and fixes remain pending.

The user explicitly authorized Stage 18 execution from `PHASE-23-STAGE-18-MULTIPLAYER-GO-FINAL-PUZZLE-AND-SOLO-PRACTICE-GO-HARD-MODE-FIXES-SPEC-2026-06-08.md`.

#### 28.63.1 Protected Starting State

- Active branch at kickoff: `main`.
- The local worktree is intentionally dirty with Stage 18 planning/governance state:
  - modified `AGENT-IMPLEMENTATION-PLAN.md`, `CHANGELOG.md`, `agents.md`, `memory.md`, and `progress/PROGRESS.csv`
  - untracked Stage 18 spec file
  - untracked `progress/PROGRESS-STEP-134.md`
- The current local worktree remains the source of truth.
...
#### 28.63.2 Baseline Resource Snapshot

Baseline was captured before any Stage 18 dev-server/browser testing:

- No local Vite/dev-server listener was found on the usual app ports `5173`, `5174`, `3000`, or `4173`.
- One unrelated Python listener was already present on `127.0.0.1:8765` (PID `26051`).
- Existing user/system processes included Codex, Chrome, VS Code, Finder, and other unrelated apps.
- `top` reported high pre-existing memory pressure: `17G used`, `123M unused`, and approximately `7047M` compressor.
- No Stage 18-owned dev server, Playwright run, or browser process was running at kickoff.

Stage 18 execution must use one dev server unless clearly necessary, keep browser contexts minimal, close contexts after verification, and avoid parallel heavy gates.

#### 28.63.3 Reproduction And Verification Plan

Before source-code fixes, Stage 18 must reproduce the relevant bug being fixed:

1. **Multiplayer GO final-puzzle premature termination**: prioritize this first, using real two-client Supabase-backed browser E2E for Practice Multiplayer GO and verifying Daily Multiplayer GO risk/behavior where practical. Record puzzle index, wrong-guess sequence, terminal status/result fields, and remote row state.
2. **Multiplayer GO final solved-row hold**: compare puzzles 1-4 solved-row hold behavior to puzzle 5, then verify both Practice Multiplayer GO and Daily Multiplayer GO receive the same terminal-puzzle hold/transition.
3. **Solo Practice GO Hard Mode checkbox**: reproduce the non-toggleable checkbox in Solo Practice GO, compare with Solo Practice OG wiring, and verify toggle-on/toggle-off plus first-guess enforcement.

Each fix must be small, targeted, and followed immediately by focused verification. Final verification must include focused changed-area tests, wider regressions, the full local gate, responsive smoke, real two-client Supabase-backed E2E for Multiplayer GO claims, remote probes/cleanup, Solo Practice OG non-regression, Stage 12-17 invariant checks, and final resource/process sanity checks.

No source-code fixes have been made at this checkpoint.

### 28.64 Stage 18 Final Verification And Handoff — Multiplayer GO Final Puzzle Behavior + Solo Practice GO Hard Mode Checkbox (`phase_id = 136`)

**Status**: Completed for user review. PR creation, merge, release, production deployment, Phase 24 work, and out-of-scope work remain gated.

Stage 18 reproduced and fixed the three scoped defects from `PHASE-23-STAGE-18-MULTIPLAYER-GO-FINAL-PUZZLE-AND-SOLO-PRACTICE-GO-HARD-MODE-FIXES-SPEC-2026-06-08.md`.

#### 28.64.1 Reproduction And Fix Summary

- **Multiplayer GO final puzzle premature termination**: focused domain coverage first reproduced the bug for both Practice and Daily Multiplayer GO; after solving puzzles 1-4, four wrong final-puzzle guesses caused `status = won` instead of remaining `playing`. The fix extends the final GO puzzle attempt budget inside the multiplayer submission path only, so puzzle 5 continues alternating until a correct solve while non-final puzzle advancement and scoring/rating rules remain unchanged.
- **Multiplayer GO final solved-row hold**: focused panel coverage was strengthened to cover terminal-puzzle completion after multiple wrong final guesses for both Practice and Daily Multiplayer GO. The existing solved-row hold path now appears after puzzle 5 before terminal definitions/results, matching the puzzles 1-4 treatment.
- **Solo Practice GO Hard Mode checkbox**: focused Solo Practice GO coverage reproduced the fresh-chain bug: the Hard Mode checkbox was disabled before any actual submitted guess. The fix uses the Stage 17 Practice GO submitted-guess predicate for the Hard Mode lock condition, so Hard Mode stays toggleable on fresh Practice GO chains, locks after the first real guess, and remains unchanged for Daily GO and Solo Practice OG.

#### 28.64.2 Focused Verification
...
#### 28.64.3 Real Browser And Supabase Evidence
...
- **Practice Multiplayer GO**: after solving the first four puzzles, the final puzzle remained `playing` after wrong guesses `slate`, `crane`, `brisk`, and `cigar`; remote probe showed nine durable moves after the final correct solve, both `playerSessions` on puzzle index 4, and final status `won`. A second Practice run confirmed the immediate browser state included `Advancing to final results` after the final solve before terminal definitions.
- **Daily Multiplayer GO**: after solving the first four puzzles, the final puzzle remained `playing` after wrong guesses `slate`, `crane`, `brisk`, and `cigar`; the final correct solve produced `status = won`, both player sessions included the final solution row, and the browser reported `Advancing to final results` with terminal definitions still withheld during the hold.
- Temporary multiplayer rows, Daily claims, and temporary auth users created for Stage 18 E2E were cleaned up.

#### 28.64.4 Full Gate And Resource Notes
...
- `npx tsc -p tsconfig.api.json --noEmit`
- `git diff --check`
- Desktop, tablet-like, and 390px browser smoke with no new console errors and no horizontal overflow

One Vite dev server was used for browser verification and then stopped. Playwright CLI sessions were closed, generated `.playwright-cli/` artifacts were removed, temporary Supabase data/users were cleaned, and final resource checks found no Stage 18-owned runaway process.

Stage 18 preserved Stage 12 through Stage 17 wins, Daily Multiplayer invariants, `playerSessions` as canonical per-viewer state, shared `serializedSession` as compatibility/answer plumbing only, Solo Practice OG behavior, Daily OG/GO determinism, and scoring/rating rules. No PR, merge, release, production deployment, full dedicated Multiplayer tab work, spectator expansion, notifications, floating manager, bots/social/export work, broad refactor, redesign, Phase 24 work, or out-of-scope work was performed.

### 28.65 Stage 19 Planning — Solo/Daily GO Transition Screen, Keyboard Coloring, and Multiplayer GO Transition Propagation Bug Fixes (`phase_id = 137`)

**Status**: Planning/governance-only. Implementation remains gated until the user explicitly authorizes Stage 19 execution.

**Binding source of truth**: `PHASE-23-STAGE-19-SOLO-AND-DAILY-GO-TRANSITION-AND-KEYBOARD-BUGFIXES-SPEC-2026-06-09.md`, with supporting bug-report context in `phase23_stage19_bugs.md`.

Stage 19 is planned as a narrow, targeted, GO bug-fix-only pass before Phase 23 can close. It covers exactly three reported defects:

1. **Missing solo GO transition screen and sound**: solo Practice GO and solo Daily GO do not show the all-green transition screen or confirming sound after a correct GO puzzle solve. Future execution must mirror the existing Multiplayer GO solved-row hold pattern as narrowly as possible for solo Practice/Daily GO.
2. **Daily GO final-puzzle keyboard coloring**: Daily GO final puzzle keyboard colors do not consistently enforce the canonical board-evidence precedence rules in solo and multiplayer Daily GO. Green board evidence must win, orange evidence must apply when no same-letter green exists, and gray evidence must apply only when no same-letter green/orange exists.
3. **Multiplayer GO asymmetric transition propagation**: after a later GO puzzle solve, especially puzzle 4 in a five-puzzle chain after many guesses, one player can advance while the other remains stuck on the transition screen, effectively locking that player out until terminal completion. Future execution must reproduce and fix this for Practice Multiplayer GO and verify Daily Multiplayer GO.

#### 28.65.1 Strict Scope Boundaries

Stage 19 execution, if later authorized, must stay inside the three defects above.

Explicitly out of scope:

- Solo Practice OG, Daily OG, and any OG-mode behavior.
- Practice Multiplayer GO or Daily Multiplayer GO changes except the specific transition propagation and keyboard coloring fixes described in the Stage 19 spec.
- GO chain advancement rule changes for non-final puzzles.
- Hard Mode enforcement, Customize box behavior, resume behavior, scoring/rating/ELO, Daily determinism, authenticated Practice seed behavior, or the unified Multiplayer model.
- UI layout, styling, copy, theming, or visual polish work beyond the existing transition screen/sound behavior needed for Bug 1.
- New features, spectator expansion, dedicated Multiplayer tab work, notifications, bots, social features, exports, Phase 24 work, broad `src/game/go/`, `src/multiplayer/`, session-management, or projection refactors.
- PR creation, merge, release, production deployment, or later-phase work.

If investigation shows that Bug 2 or Bug 3 requires touching broader or out-of-scope surfaces, execution must stop and report rather than broadening scope.

#### 28.65.2 Invariants To Preserve

Stage 19 must preserve all Stage 12 through Stage 18 wins, including:
...
- GO prior-solution visibility, authenticated Practice seeds, Daily OG/GO global determinism, Practice Multiplayer GO prior-solution keyboard projection, Solo Practice GO Customize locking, Stage 18 final-puzzle continuation/final solved-row hold behavior, and Solo Practice GO Hard Mode toggling.

Daily Multiplayer remains strictly asynchronous, fixed at five letters, UTC-day keyed, no-clock, no-Hard-Mode-lobby-control, answer-separated, and claim-safe. `playerSessions` remain canonical per-viewer state, and shared `serializedSession` remains compatibility/answer plumbing only. Scoring/result settlement rules must not change.

#### 28.65.3 Required Execution Discipline And Verification

If Stage 19 execution is later authorized:

- Reproduce all three bugs before source-code fixes, with real two-client Supabase-backed browser E2E for the Multiplayer GO propagation bug.
- Study the Stage 18 Multiplayer GO transition/solved-row hold implementation before applying any solo GO transition changes.
- Make one small targeted change at a time, followed immediately by focused verification.
- Use real two-client Supabase-backed browser E2E for Practice Multiplayer GO and Daily Multiplayer GO claims.
- Pair multiplayer browser evidence with remote Supabase probes and cleanup of temporary rows/users.
- Finish with focused changed-area tests, wider GO regressions, `npm run lint`, `npm run test`, `npm run build`, `npx tsc -p tsconfig.api.json --noEmit`, `git diff --check`, desktop/tablet/390px browser smoke, solo Practice/Daily GO transition/sound checks, Stage 12-18 invariant checks, and resource/process sanity checks.

This planning pass made no source-code, test, UI/component, Supabase migration, configuration, implementation-branch, PR, merge, release, production deployment, browser-verification, or Phase 24 changes.

### 28.66 Stage 19 Execution Kickoff — Protected State, Baseline Resources, And Reproduction Plan (`phase_id = 138`)

**Status**: Execution opened after explicit user authorization. Reproduction and fixes are pending.

**Binding source of truth**: `PHASE-23-STAGE-19-SOLO-AND-DAILY-GO-TRANSITION-AND-KEYBOARD-BUGFIXES-SPEC-2026-06-09.md`, with supporting bug-report context in `phase23_stage19_bugs.md`.

The protected starting state is the current local `main` worktree with Stage 19 planning/governance dirt preserved. The initial `git status --short` showed the existing Stage 19 governance changes in `AGENT-IMPLEMENTATION-PLAN.md`, `CHANGELOG.md`, `agents.md`, `memory.md`, `progress/PROGRESS.csv`, plus the uploaded Stage 19 spec, supporting bug note, and `progress/PROGRESS-STEP-137.md`.

Baseline resource checks before dev-server/browser testing found no listeners on the usual Vite/app ports `5173`, `5174`, `3000`, or `4173`. Existing unrelated Python listeners were present on localhost ports including `8742`, `8765`, `9000`-`9004`, and `9039`-`9048`. The memory snapshot showed high pre-existing pressure: `17G used`, `315M unused`, and about `7426M` compressor in `top`, with `vm_stat` reporting `2118539` pages stored in compressor and `475356` pages occupied by compressor.

Stage 19 remains limited to the three scoped bugs:

1. Multiplayer GO asymmetric transition/stuck-player propagation, prioritized first and requiring real two-client Supabase-backed browser E2E.
2. Daily GO final-puzzle keyboard coloring in solo and multiplayer.
3. Solo Practice/Daily GO missing solved-row transition screen and sound.

Execution must reproduce the relevant bug before any source fix, make one small targeted change at a time, run focused verification immediately after each logical change, and preserve all Stage 12 through Stage 18 wins plus Daily Multiplayer invariants. No source-code fixes have been made at this kickoff checkpoint.

### 28.67 Stage 19 Focused Reproduction And Fixes — GO Transition Propagation, Daily Keyboard Evidence, And Solo Holds (`phase_id = 139`)

**Status**: Focused fixes implemented and changed-area verification passing. Full gate, real multiplayer E2E, remote probes/cleanup, responsive smoke, final resource check, and final handoff remain pending.

Stage 19 reproduction and focused fixes stayed inside the three approved GO bugs:

1. **Multiplayer GO asymmetric transition/stuck-player propagation**: focused `multiplayer` regressions reproduced the bug for both Practice and Daily GO by solving puzzles 1-3, exhausting player two's own puzzle-4 attempts, then having player one solve puzzle 4. Before the fix, the solving player kept the turn and the exhausted player stayed non-playing. The targeted domain fix keeps `playerSessions` canonical but allows a shared solved GO move to recover/advance the non-solving player's same-puzzle session even if that canonical board had already exhausted attempts. The regression now also proves the recovered player can submit a puzzle-5 move, preserving multiplayer turn flow.
2. **Daily Multiplayer GO final-puzzle keyboard coloring**: a focused `MultiplayerGameSurface` regression reproduced that Daily GO keyboard colors ignored prior-solution evidence when a final-puzzle shared move was projected. The targeted projection fix applies the existing GO display-evidence merge to Daily GO as well as Practice GO, then derives keyboard colors from the merged visible rows using the existing `green > orange > gray` precedence helper. The solo Daily GO final-puzzle keyboard path was checked in the browser after solving the first four 2026-06-09 Daily GO puzzles and was not reproduced: visible prior evidence colored the final-puzzle keyboard correctly.
3. **Solo Practice/Daily GO transition screen and sound**: live browser checks reproduced that solo Practice GO and solo Daily GO immediately advanced from puzzle 1 to puzzle 2 after correct solves, without a held solved-row transition. The targeted `GoGame` fix adds a solo solved-row transition detector and a 2-second hold patterned after Multiplayer GO, disables input during the hold, renders the just-solved puzzle, and plays the `correct-guess` cue for any solved GO puzzle rather than only whole-chain wins.

Focused verification completed at this checkpoint:

- `npx vitest run src/multiplayer/multiplayer.test.ts src/multiplayer/MultiplayerGameSurface.test.tsx` passed after the multiplayer/domain/projection fixes.
- `npx vitest run src/app/games/soloHardModeDefaults.test.tsx` passed after the solo transition helper/fix.
- `npx vitest run src/app/games/soloHardModeDefaults.test.tsx src/multiplayer/multiplayer.test.ts src/multiplayer/MultiplayerGameSurface.test.tsx` passed with 41 tests.
- Browser verification confirmed fresh Practice GO shows `Advancing to the next puzzle.` immediately after solving `abbes`, then advances to puzzle 2 after about 2.2 seconds; a sound-engine stub recorded playback during the solved transition.
- Browser verification confirmed fresh Daily GO on 2026-06-09 shows `Advancing to the next puzzle.` immediately after solving `cable`, then advances to puzzle 2 after about 2.2 seconds.
- Browser verification checked solo Daily GO final puzzle keyboard state after solving `cable`, `cades`, `cadet`, and `cadis`; C/A/D/E were colored green and B/L/S/T/I gray from visible evidence, so no solo keyboard code change was made.

Pending work: real two-client Supabase-backed Practice/Daily Multiplayer GO E2E, remote Supabase probes/cleanup, wider GO regressions, full lint/test/build/API typecheck/diff gate, desktop/tablet/390px smoke, final resource/process check, final progress report, and user-review handoff.

### 28.68 Stage 19 Final Verification And Handoff — Solo/Daily GO Transition, Daily Keyboard Evidence, And Multiplayer GO Propagation (`phase_id = 140`)

**Status**: Completed for user review. PR creation, merge, release, production deployment, full dedicated Multiplayer tab work, spectator expansion, Phase 24 work, and out-of-scope work remain gated.

Stage 19 completed the narrow three-bug GO pass from `PHASE-23-STAGE-19-SOLO-AND-DAILY-GO-TRANSITION-AND-KEYBOARD-BUGFIXES-SPEC-2026-06-09.md`, with supporting context from `phase23_stage19_bugs.md`.

#### 28.68.1 Reproduction And Fix Summary

- **Multiplayer GO asymmetric transition/stuck-player propagation**: focused domain coverage reproduced the bug before the fix for both Practice and Daily GO. After puzzles 1-3 were solved, the non-solving player could exhaust the puzzle-4 canonical session, then the rival's correct puzzle-4 solve failed to advance that exhausted player to puzzle 5. The targeted fix recovers only the non-solving player's same-puzzle GO session when a shared all-correct solved move proves the puzzle was solved, preserving canonical `playerSessions` and avoiding ordinary rival-guess copying.
- **Daily GO final-puzzle keyboard coloring**: focused component coverage reproduced the Daily Multiplayer GO keyboard projection bug before the fix. Daily GO now uses merged visible GO evidence for display and keyboard colors, matching the existing Practice GO evidence path and preserving the existing `green > orange > gray` precedence helper. Solo Daily GO final-puzzle keyboard coloring was checked in-browser after solving the first four current Daily GO puzzles and was not reproduced, so no solo keyboard helper change was made.
- **Solo Practice/Daily GO transition screen and sound**: browser checks reproduced missing solo Practice GO and solo Daily GO solved-row holds before the fix. `GoGame` now detects solo solved-puzzle transitions, holds the just-solved all-green puzzle for about two seconds, disables input during the hold, then advances normally while triggering the existing `correct-guess` sound cue after a user gesture.

#### 28.68.2 Focused And Full Verification

Verification passed:

- Focused changed-area tests: `npx vitest run src/app/games/soloHardModeDefaults.test.tsx src/multiplayer/multiplayer.test.ts src/multiplayer/MultiplayerGameSurface.test.tsx` passed 41 tests.
- Wider GO regression set: 12 files / 99 tests passed.
- `npm run lint`
- `npm run test` — 493 tests passing
- `npm run build` — succeeded with the existing large-chunk advisory
- `npx tsc -p tsconfig.api.json --noEmit`
- `git diff --check`
- Desktop, tablet-like, and 390px browser smoke passed with no new console errors and no horizontal overflow.

#### 28.68.3 Real Browser And Supabase Evidence

Real browser-backed two-client Supabase E2E passed with isolated temporary authenticated users:

- **Practice Multiplayer GO**: after a puzzle-4 solve where the non-solving player had exhausted that puzzle, both `playerSessions` advanced to puzzle index 4, `status` remained `playing`, the next turn belonged to the recovered rival, the final puzzle stayed multiplayer after a wrong guess, and the final correct solve completed with `status = won`.
- **Daily Multiplayer GO**: the same puzzle-4 recovery and final-puzzle continuation path passed against Daily Multiplayer GO. Daily claim rows, participant ids, durable moves, per-player sessions, current turn, and terminal status were remotely probed.
- **Cleanup**: the two temporary auth users, two touched `async_multiplayer_games` rows, and two Daily claim rows created for Stage 19 verification were deleted. A follow-up probe confirmed no Stage 19 E2E users, rows, or claims remained.

Solo browser verification confirmed:

- Solo Practice GO shows the solved all-green row with `Advancing to the next puzzle.` immediately after solving, then advances after the hold.
- Solo Daily GO shows the same solved-row transition for the current Daily GO chain.
- The sound engine path was reached during the solved transition after a user gesture.
- Solo Daily GO final-puzzle keyboard evidence reflected the visible prior rows in the checked run.

#### 28.68.4 Resource And Scope Notes

One Vite dev server was used for browser verification and then stopped. Playwright browser contexts were closed, `.playwright-cli/` generated artifacts were removed, and the final process snapshot showed no Stage 19-owned listener on `5173`, `5174`, `3000`, or `4173` and no Stage 19-owned runaway browser/dev-server process. The machine still showed high memory pressure from unrelated pre-existing user/system processes.

Stage 19 preserved Stage 12 through Stage 18 wins, Daily Multiplayer invariants, `playerSessions` as canonical per-viewer state, shared `serializedSession` as compatibility/answer plumbing only, Solo Practice OG behavior, Daily OG/GO determinism, scoring/result settlement rules, Stage 18 final-puzzle continuation, and Stage 18 final solved-row holds. No PR, merge, release, production deployment, full dedicated Multiplayer tab implementation, spectator expansion, notifications, floating manager, bots/social/export work, scoring/rating changes, broad refactor, redesign, Phase 24 work, or out-of-scope work was performed.

### 28.69 Stage 20 Planning — Multiplayer Status Text Synchronization + Forfeit Win/Loss Precedence (`phase_id = 141`)

**Status**: Planning/governance-only. Implementation remains gated until the user explicitly authorizes Stage 20 execution.

**Binding source of truth**: `PHASE-23-STAGE-20-MULTIPLAYER-STATUS-TEXT-AND-FORFEIT-LOGIC-BUGFIXES-SPEC-2026-06-09.md`.

Stage 20 is an extremely narrow, bug-fix-only pass inside Phase 23. It targets exactly two unified Multiplayer defects across OG/GO and Practice/Daily variants:

1. **Multiplayer status text synchronization**: the small status/message box above the turn history can remain stale on one client after lobby open, join, turn submission, forfeit, timeout, or normal completion events. Future execution must make this status reactive to the current shared game state from the viewing player's perspective on both clients.
2. **Forfeit win/loss precedence**: when a player with more points forfeits, the current result settlement can incorrectly declare that forfeiting player the winner on points. Future execution must make post-guess forfeits lose for the forfeiting player regardless of point totals. The narrow pre-guess exception remains allowed: forfeiting before any submitted guesses may be treated as a non-result/cancellation. Existing timeout behavior already makes the timed-out player lose and must remain unchanged.

#### 28.69.1 Strict Scope Boundaries

Stage 20 execution, if later authorized, must stay inside the two defects above.

In scope:

- Multiplayer status/message text and its shared-state-derived update logic.
- Forfeit-specific result precedence and any minimal supporting projection/event state needed for that precedence.
- Focused tests and real two-client Supabase-backed browser verification for the affected multiplayer flows.

Explicitly out of scope:

- Gameplay board, letter tiles, keyboard behavior, tile coloring, Hard Mode validation during play, solved-row holds/transitions, core game mechanics, and GO/OG advancement rules.
- Scoring or point-calculation formula changes; only the final win/loss decision for forfeits may change.
- Daily Multiplayer rule changes beyond preserving the existing claim-safe async behavior.
- New features, UI redesign, full dedicated Multiplayer tab work, spectator expansion, notifications, floating manager, bots, exports, Phase 24 work, broad refactoring, performance work, PR creation, merge, release, or production deployment.

If investigation shows that either bug requires broad architecture, scoring/rating, game-board, keyboard, or out-of-scope mode changes, execution must stop and report rather than broadening scope.

#### 28.69.2 Invariants To Preserve

Stage 20 must preserve all Stage 12 through Stage 19 wins, including Practice Multiplayer Hard Mode enforcement, keyboard responsiveness, sound playback, stale-save protections, timed Practice clock rules, row-write reduction, result settlement protections, GO solved-row hold/advance behavior, authenticated Practice seed behavior, hidden/inert Multiplayer foundations, nonparticipant guard, Practice Multiplayer GO projection fixes, Solo Practice GO Customize/Hard Mode fixes, Stage 18 final-puzzle continuation/final solved-row hold behavior, and Stage 19 solo/Daily GO transition plus Multiplayer GO propagation fixes.

Daily Multiplayer remains strictly asynchronous, fixed at five letters, UTC-day keyed, no-clock, no-Hard-Mode-lobby-control, answer-separated, and claim-safe. `playerSessions` remain canonical per-viewer state, and shared projections remain display/compatibility plumbing only.

#### 28.69.3 Required Execution Discipline And Verification

If Stage 20 execution is later authorized:

- Reproduce both bugs before source-code fixes, ideally with real two-client Supabase-backed browser E2E.
- Make one small targeted change at a time, followed immediately by focused verification.
- Use real two-client Supabase-backed browser E2E plus remote Supabase probes and cleanup for multiplayer state claims.
- Cover OG and GO, Practice and Daily where the bug applies, including join, turn submission, forfeit, timeout non-regression, and normal completion.
- Finish with focused changed-area tests, wider multiplayer/GO regressions, `npm run lint`, `npm run test`, `npm run build`, `npx tsc -p tsconfig.api.json --noEmit`, `git diff --check`, desktop/tablet/390px browser smoke, resource/process sanity checks, and optional Vercel preview if useful for review.

This planning pass made no source-code, test, UI/component, Supabase migration, configuration, implementation-branch, PR, merge, release, production deployment, browser-verification, or Phase 24 changes.

### 28.70 Stage 20 Execution Kickoff — Protected State, Baseline Resources, And Reproduction Plan (`phase_id = 142`)

**Status**: Stage 20 execution opened after explicit user authorization. Source-code fixes remain pending until both scoped bugs are reproduced.

**Binding source of truth**: `PHASE-23-STAGE-20-MULTIPLAYER-STATUS-TEXT-AND-FORFEIT-LOGIC-BUGFIXES-SPEC-2026-06-09.md`, `progress/PROGRESS-STEP-141.md`, this §28.69 planning section, `CONSTITUTION.md`, `agents.md`, `memory.md`, and the Stage 19 final handoff at `progress/PROGRESS-STEP-140.md`.

#### 28.70.1 Protected Starting State

- Active branch: `main`.
- Starting worktree: clean local `main` after PR #23 merged Stage 19 final changes and Stage 20 planning.
- No reset, rebase, pull-over, branch switch away, discard, force-push, branch deletion, PR creation, merge, release, production deployment, full dedicated Multiplayer tab work, spectator expansion, Phase 24 work, or out-of-scope work is authorized.

#### 28.70.2 Baseline Resource Snapshot

Before dev-server/browser testing, the checkpoint recorded:
...
- Memory pressure was high before Stage 20 browser work (`17G used`, about `6930M` compressor, `540M` unused).

Stage 20 must use one Vite dev server unless a clear reason appears, keep browser contexts minimal, close contexts promptly, avoid parallel heavy gates, and confirm no Stage 20-owned runaway processes at handoff.

#### 28.70.3 Reproduction Plan

Stage 20 execution must reproduce both bugs before source fixes:

1. **Status text synchronization**: use real two-client Supabase-backed browser E2E with isolated authenticated contexts to compare each client's status box after lobby open, join, turn submission, normal completion, forfeit, and timeout non-regression where practical across OG/GO and Practice/Daily surfaces.
2. **Forfeit win/loss precedence**: reproduce a post-guess forfeit by a points-leading player and confirm the current broken winner/loser outcome before fixing. Also verify the pre-guess non-result/cancellation exception and existing timeout-loser behavior.

Focused tests should be added around the smallest status-text and forfeit-result seams before production fixes. Future browser claims must be paired with remote Supabase probes and cleanup.

#### 28.70.4 Strict Scope And Verification Plan

Authorized changes remain limited to:

- Multiplayer status/message text synchronization.
- Forfeit-specific final win/loss precedence.
- Minimal supporting state/projection/event/test changes needed for those two bugs.

Explicitly out of scope: gameplay board, letter tiles, keyboard, tile coloring, Hard Mode validation, solved-row hold/transition behavior, GO/OG advancement, scoring formulas, rating/ELO changes, Daily Multiplayer rule changes, UI redesign, new features, full Multiplayer tab work, spectator expansion, broad refactoring, PR creation, merge, release, production deployment, Phase 24 work, and later-phase work.

Verification must include focused changed-area tests, wider multiplayer/GO regressions, full lint/test/build/API typecheck/diff gate, responsive browser smoke, real two-client Supabase-backed browser E2E for affected OG/GO Practice/Daily flows, remote probes/cleanup, timeout non-regression, Stage 12-19 invariant preservation, and final resource/process checks.

### 28.71 Stage 20 Focused Reproduction And Fixes — Status Text And Forfeit Precedence (`phase_id = 143`)

**Status**: Focused reproduction and targeted fixes complete. Final real E2E, full gate, responsive smoke, resource check, optional preview, and final handoff remain pending.

#### 28.71.1 Reproduction Evidence

Focused tests were added before source fixes and failed against the original behavior:

- `MultiplayerPanel` status text regressions showed a creator's status stayed stale after a rival joined and a rival's status stayed stale after the creator submitted a turn.
- Multiplayer domain/scoring regressions showed a pre-guess forfeit was still treated as a loss, and a post-guess forfeit could still project as a points win for the forfeiting player when durable winner evidence was absent or insufficient.
- Timeout non-regression coverage was added to confirm the timed-out player still loses regardless of points.

#### 28.71.2 Targeted Fixes

- The multiplayer status/message box now derives its default text from the current shared game state and viewer identity, including waiting, joined, active-turn, submitted-turn, cancellation, forfeit, timeout, and normal terminal states.
- Local success messages that can go stale after shared multiplayer state changes are cleared so both clients reconcile to the same state-derived status text.
- Multiplayer game state now records `forfeitedPlayerId` for post-guess forfeits, normalizes that durable evidence when rows are reloaded, and treats pre-guess forfeits as cancellations without a winner.
- Multiplayer result projection now gives post-guess forfeit evidence precedence over points fallback while preserving existing timeout-loser precedence and leaving scoring formulas unchanged.

#### 28.71.3 Verification Completed At This Checkpoint

- `npm run test -- src/multiplayer/multiplayer.test.ts src/multiplayer/MultiplayerPanel.test.tsx src/multiplayer/scoring.test.ts` passed after the fixes.
- `npm run test -- src/multiplayer` passed after the fixes.

#### 28.71.4 Remaining Verification

The Stage 20 final gate remains open. Required next steps are real two-client Supabase-backed browser E2E for OG/GO Practice/Daily affected flows, remote probes/cleanup, timeout non-regression proof in an integrated flow where practical, full lint/test/build/API typecheck/diff gate, desktop/tablet/390px responsive smoke, final resource/process snapshot, optional preview, and final progress handoff.

### 28.72 Stage 20 Final Verification And Handoff — Status Text Synchronization And Forfeit Precedence (`phase_id = 144`)

**Status**: Completed for user review. PR creation, merge, release, production deployment, full dedicated Multiplayer tab work, spectator expansion, Phase 24 work, and out-of-scope work remain gated.

Stage 20 completed the narrow two-bug multiplayer pass from `PHASE-23-STAGE-20-MULTIPLAYER-STATUS-TEXT-AND-FORFEIT-LOGIC-BUGFIXES-SPEC-2026-06-09.md`.

#### 28.72.1 Reproduction And Fix Summary

- **Status text synchronization**: focused regressions reproduced stale status text before source fixes. The status/message box now derives its default text from the current shared `MultiplayerGame` state and viewer identity for waiting, join, active turn, submitted turn, cancellation, forfeit, timeout, and normal terminal states. Local validation/error messages are snapshot-scoped so they no longer mask newer shared multiplayer state.
- **Forfeit win/loss precedence**: focused regressions reproduced post-guess forfeit projections where a forfeiting player could still win on points and pre-guess forfeits were still recorded as losses. Multiplayer games now persist/normalize `forfeitedPlayerId`; pre-guess forfeits cancel without a winner; post-guess forfeits resolve against the forfeiting player before any points fallback. Timeout-loser precedence and scoring formulas are unchanged.

#### 28.72.2 Focused And Full Verification

Verification passed:

- Focused changed-area tests: `npm run test -- src/multiplayer/multiplayer.test.ts src/multiplayer/MultiplayerPanel.test.tsx src/multiplayer/scoring.test.ts` passed 45 tests.
- Wider multiplayer regression set: `npm run test -- src/multiplayer` passed 78 tests.
- `npm run lint`
- `npm run test` — 499 tests passing
- `npm run build` — succeeded with the existing large-chunk advisory
- `npx tsc -p tsconfig.api.json --noEmit`
- `git diff --check`
- Desktop, tablet-like, and 390px browser smoke passed with no console/page errors and no horizontal overflow.

#### 28.72.3 Real Browser And Supabase Evidence

Real two-client Supabase-backed browser E2E passed with isolated temporary authenticated users:

- **Practice OG**: verified open, join, turn submission, normal completion, and both players' synchronized status messages.
- **Practice GO**: verified open, join, turn submission, post-guess forfeit by the points-leading rival, and both players' terminal forfeit messages.
- **Pre-guess cancellation**: verified a joined Practice OG lobby with zero moves cancels without a winner when forfeited before any guess.
- **Daily OG**: verified open, join, turn submission, post-guess forfeit, and forfeiter-loses status/projection behavior.
- **Daily GO**: verified open, join, turn submission, full five-puzzle normal completion, and synchronized terminal status.
- **Timed Practice timeout non-regression**: verified the timed-out player still loses regardless of points and both players receive timeout-specific status text.
- **Remote probes and cleanup**: probed durable rows, participants, moves, per-player sessions, `forfeitedPlayerId`, `timedOutPlayerId`, winner/status fields, and Daily claim rows before cleanup. Touched `async_multiplayer_games` rows, Daily claims, and temporary auth users were deleted, and a follow-up probe confirmed no touched multiplayer rows remained.
...
#### 28.72.4 Resource And Scope Notes

One Vite dev server was used for browser verification and then stopped. Temporary Playwright scripts and generated browser artifacts were removed. The final resource snapshot showed high pre-existing memory pressure from unrelated user/system processes; the Stage 20 Vite process was stopped afterward and port `5173` was confirmed clear. No Stage 20-owned runaway browser/dev-server process remained.

Stage 20 preserved Stage 12 through Stage 19 wins, Daily Multiplayer invariants, `playerSessions` as canonical per-viewer state, shared projections as display/compatibility plumbing only, timeout-loser precedence, and scoring formulas. No gameplay board/tile/keyboard/coloring changes, Hard Mode validation changes, solved-row hold changes, advancement-rule changes, scoring formula/rating/ELO changes, Daily Multiplayer rule changes, PR, merge, release, production deployment, full dedicated Multiplayer tab implementation, spectator expansion, notifications, floating manager, bots/social/export work, Phase 24 work, broad refactor, redesign, or out-of-scope work was performed.
