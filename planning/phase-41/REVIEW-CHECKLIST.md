...
User-only manual review remains required for the checklist below.

## Must Manually Verify

- [ ] Ranked Practice search-again reliably creates and opens the next game.
...
- [ ] Cancelled ranked queue participation does not later match stale entries.
...
- [ ] Public ranked leaderboard refreshes after a trusted ranked Practice result.
...
- [ ] Private Practice request cancel/decline lifecycle cleans active lists.
...
- [ ] Private Practice request accept flow gives requester a safe open/resume path.
...
- [ ] Mobile Practice Multiplayer freshness is improved.
...
- [ ] Phase 41 visual handoff artifacts remain local-only and ignored.
...
- [ ] Try a ranked Practice queue cancellation followed by immediate re-entry on the same account.
  - Expected: the app uses the current queue request and does not surface stale cancelled state.

- [ ] Try a private request cancel or decline on a mobile browser after backgrounding and refocusing the tab.
  - Expected: route/focus/visibility refresh updates the request list without exposing private fields.

- [ ] Spot-check desktop and mobile layouts for Multiplayer -> Practice, Multiplayer -> Lobby, and Leaderboard.
  - Expected: text fits, controls remain reachable, and no UI overlaps or overflows.

- [ ] Review Phase 41 visual screenshots if the local artifact directory is available.
...
- [ ] Phase 40 public profile route/card, clickable safe identity, and private matchmaking boundaries remain intact.
...
- [ ] Any failed item has a follow-up prompt prepared before Phase 41 Git handoff or additional implementation.
