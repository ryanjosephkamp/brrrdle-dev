...
- [ ] Mobile Practice Solo GO `New go chain` scrolls to a usable keyboard.
...
- [ ] Mobile Daily Solo GO remains playable after a submitted guess and re-entry.
...
- [ ] Mobile Practice Solo GO remains playable after a submitted guess and re-entry.
...
- [ ] Mobile Solo OG behavior did not regress.
...
- [ ] Mobile back-to-top control does not block Solo keyboard use.
...
- [ ] Signed-out History does not show the just-signed-out account's private multiplayer rows.
...
- [ ] Signed-out Leaderboard does not show the just-signed-out account's local rating summary.
...
- [ ] Signed-out Dashboard/Home and Active Multiplayer do not show stale account-owned multiplayer attention.
...
- [ ] Stale authenticated sync completion does not repopulate guest-visible account data after sign-out.
...
- [ ] Phase 47 visual handoff artifacts remain local-only and ignored.
...
- [ ] Any failed item has a follow-up prompt prepared before Phase 48 planning or additional implementation.
