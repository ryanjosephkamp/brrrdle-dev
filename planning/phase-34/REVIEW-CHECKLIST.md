...
- [ ] The selected `Live` subtab count is readable.
...
- [ ] Live participant cards use safe player names rather than incorrect opponent `You` labels.
...
- [ ] Authenticated Live spectator cards use safe profile names where available.
...
- [ ] Live cards clearly show whether a game is `Ranked` or `Unranked`.
...
- [ ] Joinable Lobby rows use a direct `Join` action.
...
- [ ] Lobby `Join` joins the match in one guarded click.
...
- [ ] Lobby one-click join does not create a duplicate lobby.
...
- [ ] In-app `Your turn` notifications route directly to the relevant active game when safe.
...
- [ ] Foreground browser notification clicks follow the same safe direct-resume path.
...
- [ ] Stale notification targets fall back safely to Multiplayer -> Active Games.
...
- [ ] Active Games cards visibly mark games where it is your turn.
...
- [ ] Phase 34 visual handoff artifacts remain local-only and ignored.
