...
### Plan Addendum drafted — DIAGNOSIS-REPORT-ADMIN-TAB-2026-05-27 (2026-05-27)
- A new addendum was appended to `AGENT-IMPLEMENTATION-PLAN.md` as Section 19 ("Phase 14 — Plan Addendum (DIAGNOSIS-REPORT-ADMIN-TAB-2026-05-27): Fix the Admin Tab"). The plan header version moves to 1.4.
- The addendum diagnoses the root cause of the Admin-tab regression reported in `DIAGNOSIS-REPORT-ADMIN-TAB-2026-05-27.md`: the `access.allowed` branch in `src/admin/AdminPanel.tsx` currently renders only descriptive copy (no actual "Refresh now" button), so even admin users see "only the placeholder text". Secondary hardening covers admin-role detection over both `app_metadata` and `raw_app_meta_data` (role and roles[]) and an explicit best-effort `supabase.auth.refreshSession()` after successful sign-in/sign-up and on `SIGNED_IN` / `TOKEN_REFRESHED` / `USER_UPDATED` events so the Admin tab updates immediately for both the magic-link and email + password flows.
- The addendum is split into Phase 14.0 (pre-flight & reproduction map) and Phase 14.1 (the surgical fix), each with its own verification commands, halt-for-approval gate, and progress artifact (`progress/PROGRESS-STEP-20.md`, `progress/PROGRESS-STEP-21.md`).
- Manual follow-up steps documented in the addendum: confirm at least one Supabase user has `raw_app_meta_data.role = "admin"`; confirm Email + Password auth remains enabled; no Vercel env-var or routing change; first-time admin users should sign out and back in once after deploy.
- No source code, route table, server route, configuration file, or test was modified in this addendum-drafting step. No file was deleted or renamed. Implementation of Section 19 must NOT begin without explicit user approval (CONSTITUTION §5.3, §6).
- `CHANGELOG.md` `[Unreleased] — Documentation` updated to record that the addendum has been created.
