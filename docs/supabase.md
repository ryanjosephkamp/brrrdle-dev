...
Phase 55 ranked Daily SQL/RLS authority is source-controlled by:
...
This migration preserves existing unranked claims as `ranked=false`, widens Daily uniqueness to user/transport/mode/date/ranked, and keeps legacy six-argument claim/release callers restricted to the unranked lane. The ranked-aware seven-argument helpers are trigger/internal authority and are not granted to browser roles. Ranked Daily queue creation, status, finalization, action submission, and settlement use authenticated `security definer` RPCs with empty search paths, participant ownership checks, fixed five-letter/no-clock/current-date validation, separate Daily rating buckets, strict oldest-compatible FIFO, server-generated pair reservations, deterministic lane-lock ordering, and repeat-opponent support. Practice requests continue through the existing trusted contracts.

Ranked Daily competitive state lives in the dedicated unexposed `brrrdle_private` schema. It contains the source-controlled ordered five-letter answer/valid-guess catalog, immutable pair reservations, canonical per-game authority, and an append-only idempotent action ledger. The finalizer derives the ranked answer sequence on the server and stores an answerless public projection. Direct browser insert/update access to ranked Daily game rows is denied. The action RPC accepts only a game id, optimistic authority version/move count, action id, and one guess or owned forfeit; it validates the canonical dictionary, turn, tiles, GO prefilled-attempt budget, shared Hard Mode evidence, terminal progression, and winner. Settlement reads only the private authority/action ledger for competitive evidence and keeps the existing Elo formula, K factors, and provisional threshold unchanged.

The migration set extends the authenticated public leaderboard allowlist only to `multiplayer:og:daily:v1` and `multiplayer:go:daily:v1`; timed Practice buckets remain excluded. Browser roles retain no direct insert/update/delete authority over claims, queue rows, rating profiles, transactions, or trusted results. No answers, seeds, serialized/player sessions, raw auth identifiers, private request data, queue identifiers, or settlement identifiers are added to public leaderboard payloads.

All five migrations are applied to the verified `brrrdle-dev` project and the local and remote ledgers match exactly. The additive repairs preserve the immutable base migration while supplying canonical Daily expiry, fresh request ids after terminal queue attempts, final-puzzle continuation until solve, explicit public-projection allowlisting, the queue `matched_at` field referenced by the trusted RPCs, orphan-safe authority cleanup, and reservation-owned finalization evidence. Final verification proved the expected catalog counts, restricted function search paths and grants, anonymous/private denial, concurrent FIFO behavior, temporary-account ranked Daily OG/GO gameplay and settlement, trusted leaderboard updates, dependency-safe cleanup, and zero temporary residue.
...
## Post-Phase-57 spectator termination transparency

Migration `20260711212934_post_phase57_spectator_termination_transparency.sql` replaces only the established authenticated and public Live spectator RPC bodies. It preserves both signatures, returned table shapes, stable/security-definer attributes, `public, pg_temp` search paths, grants, current-Daily exclusion, public Practice-only boundary, bounded terminal hold, and read-only capabilities. It adds two-participant pre-turn cancellation visibility plus safe `terminationReason` and optional `forfeitedSeat` outcome fields; it does not expose answers, sessions, emails, user IDs, private projections, or mutation capabilities.

The migration tool recorded generated version `20260711215831` after applying the exact source-controlled SQL once. Full schema and catalog equivalence was proven before a one-row transactional ledger-only version reconciliation to `20260711212934`; the migration SQL was not rerun. Local and remote histories now match at 39 migrations. Post-reconciliation authenticated/public RPC probes, real browser cancellation/forfeit checks, Daily Solo persistence, ranked Daily rediscovery, and the complete authority-enabled 79-test E2E suite passed with zero temporary-user/profile residue.

## Phase 58 ranked Daily GO selector v2

Migration `20260712175405_phase58_go_chain_selector_v2.sql` versions private ranked-Daily answer authority without rewriting existing games. Existing and pre-cutoff rows remain v1; Daily GO games dated on or after `2026-07-14` use v2. Public projections receive only sanctioned `answerGenerationVersion` metadata and remain answerless. Private catalogs, queue/claim/reservation/action authority, rating/Elo settlement, private requests, economy, and spectator contracts are unchanged.

The first remote v2 probe exposed signed-bigint overflow in the private avalanche mixer. The separately generated migration `20260712190338_phase58_go_chain_selector_v2_bigint_overflow_repair.sql` replaces only `brrrdle_private.phase58_mix_u32(bigint)`, using exact numeric multiplication followed by modulo `4294967296` before bigint conversion. It preserves the signature, xor/shift sequence, constants, immutability, invoker security, empty search path, ownership, and browser-role revocations.

Both migrations were applied exactly once. Supabase generated ledger versions were reconciled to their source-controlled versions only after byte-identical stored SQL and complete target-object equivalence were proven; migration SQL was not rerun. Local and remote histories now match exactly at 41/41. Scalar boundary vectors and three post-cutoff ranked/unranked date vectors match TypeScript digests, each chain contains five unique answers, and ranked/unranked sets are disjoint. Existing authority rows remain v1, public answer leaks and mixer browser grants are zero, spectator digests are unchanged, advisor scans contain no Phase 58 finding, and temporary E2E Auth/profile/projection residue is zero.
