-- Phase 35 Stage 35.3: ranked Live spectator safe identity projection.
--
-- This migration keeps the authenticated Live v1 spectator RPC signature and
-- returned table shape intact. It narrows the player profile source so
-- authenticated nonparticipant spectator rows resolve active public profile
-- summaries from async_multiplayer_games participant user ids instead of
-- trusting incomplete ranked game projection playerProfiles.
...
comment on function public.get_authenticated_live_v1_spectator_games_v2(integer, integer)
  is 'Returns sanitized authenticated-only Live v1 spectator projections with server-resolved active public profile summaries, current Daily exclusion, and bounded terminal hold rows without exposing raw projection/session/answer data.';
