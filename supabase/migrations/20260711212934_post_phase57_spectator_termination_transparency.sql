-- Post-Phase-57 review follow-up: privacy-safe spectator termination reasons.
--
-- This migration preserves both spectator RPC signatures, row shapes, role
-- grants, current-Daily exclusion, and read-only projections. It adds
-- cancelled terminal rows only when both participant seats exist and adds
-- safe outcome metadata derived from existing canonical status/projection.
...
as $$
  with authorized as (
    select
      ...
      coalesce(((select auth.jwt()) ->> 'is_anonymous')::boolean, false) as viewer_is_anonymous
  ),
  settings as (
    select
  ...
  eligible as (
    select
      game.*,
      case
        when game.status in ('won', 'lost', 'expired', 'cancelled') then coalesce(game.ended_at, game.updated_at)
        else null
      end as terminal_at,
      case
        when game.status in ('won', 'lost', 'expired', 'cancelled')
    ...
    cross join settings
    where authorized.viewer_id is not null
      and authorized.viewer_is_anonymous is false
      and game.status in ('playing', 'won', 'lost', 'expired', 'cancelled')
      ...
      and (
        game.status = 'playing'
        or (
          game.status in ('won', 'lost', 'expired', 'cancelled')
  ...
  select
    ...
    jsonb_strip_nulls(jsonb_build_object(
      'terminal', game.status in ('won', 'lost', 'expired', 'cancelled'),
      'status', game.status,
      'winnerSeat',
        case
      ...
      'forfeitedSeat',
        case
          when game.projection ->> 'forfeitedPlayerId' in ('player-one', 'player-two') then game.projection ->> 'forfeitedPlayerId'
          else null
        end,
      'terminationReason',
        case
          when game.status = 'cancelled' then 'cancelled'
          when game.projection ->> 'forfeitedPlayerId' in ('player-one', 'player-two') then 'forfeit'
          when game.status = 'expired' then 'expired'
          when game.status in ('won', 'lost') then 'completed'
          else null
        end,
      'label',
        case
          when game.status = 'playing' then 'In progress'
          when game.status = 'cancelled' then 'Match cancelled'
  ...
  cross join lateral (
    select coalesce(jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
      ...
      'profile',
        case
          when profile.public_profile_id is not null then jsonb_strip_nulls(jsonb_build_object(
...
comment on function public.get_authenticated_live_v1_spectator_games_v2(integer, integer)
  is 'Returns sanitized authenticated-only Live v1 spectator projections with safe cancellation/forfeit reasons, current Daily exclusion, and bounded terminal hold rows.';
...
comment on function public.get_public_live_v1_spectator_games_v1(integer, integer, text)
  is 'Dedicated public Practice Live spectator projection with safe cancellation/forfeit reasons, bounded terminal visibility, and read-only capabilities.';
...
-- Rollback: reapply the function bodies from migrations
-- 20260627230835_phase35_ranked_live_identity_spectator_profiles.sql and
