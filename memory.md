...
- Phase 31 implementation, final hardening, Git handoff, merge, post-merge sanity review, and feature-branch cleanup are complete; local `main` and `origin/main` were confirmed at `d4d1957fa61da14a62de2c7cf32ff50996e87523` before Phase 32 planning.
- Phase 32 is complete locally through Stage 32.7 under `phase_id = 256` pending Git handoff. It stabilized post-Phase-31 rematch lifecycle, ranked queue continuation, lobby auto-routing, opponent identity labels, account-avatar accent propagation, and no-comma Elo/rating display consistency.
- Phase 32 visual handoff review was added as an optional pre-Git-handoff human evidence gate under `phase_id = 257`: use local-only ignored screenshots/manifests under `test-results/visual-review/`, keep automated tests authoritative, and never stage screenshots, videos, traces, auth state, secrets, or local session artifacts.
- Manual phase review checklists were added as an optional committed human-review gate under `phase_id = 258`: generate `planning/phase-<N>/REVIEW-CHECKLIST.md` after final verification and any visual handoff when explicitly authorized; use the local `brrrdle-phase-review-checklist` skill as a convenience helper, and keep automated tests authoritative.
- The local `brrrdle-github-backup` skill was created under `phase_id = 259` as an explicit-authorization-only helper for all-in-one GitHub backup flows; repo-tracked skill docs live under `planning/skills/`, while actual skills remain local-only under `/Users/noir/.codex/skills/`.
- Post-Phase-34 workflow docs mirror the local `brrrdle-github-backup` routing policy: after a clean Git handoff preparation pass, generated backup prompts should normally invoke the all-in-one backup skill unless the user requests stepwise gates or forbids merge/cleanup.
- Phase 32 added participant-scoped public-safe identity summaries through `supabase/migrations/20260624233635_phase32_participant_identity_rpc.sql` and app integration that keeps opponent labels from rendering as `You` while preserving public profile default-private/moderation boundaries.
- Phase 32 final hardening added real two-client Supabase-backed E2E coverage for one-request rematch visibility/decline, eligible rematch accept/open, ranked Practice search-again routing, unranked lobby creator auto-routing, and safe opponent identity labels.
...
- Phase 30 deferred ranked-mode routing was documented under `phase_id = 226`, but Phase 32 planning under `phase_id = 246` supersedes that sequence: Phase 32 is now stabilization; Phase 33 becomes ranked mode expansion / competitive ladder v2 for timed Practice ranked first and Daily ranked only after claim-safety proof; Phase 34 moves to public/guest spectation; Phase 35 moves to theme proposal/template modernization; Phase 36 or later moves to full concrete themes.
- Post-Phase-27 roadmap routing was revised under `phase_id = 200`: public profiles moved to Phase 29, leaderboards to Phase 30, multiplayer postgame actions to Phase 31, public/guest spectation to Phase 32, theme proposal/template modernization to Phase 33, and full concrete theme implementation to Phase 34 or later. The public/guest spectation and theme phase numbers in that note were superseded by the Phase 30 deferred ranked-mode routing under `phase_id = 226`.
